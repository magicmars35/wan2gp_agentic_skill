...
# =========================================================
#
# This file is designed to live inside:
#
#   G:\APPS\Wan2GP\dgr_agentic_studio\wan2gp_api_server.py
#
# The Git repository should be initialized in:
#
#   G:\APPS\Wan2GP\dgr_agentic_studio
#
# The actual Wan2GP engine remains in the parent folder:
#
#   G:\APPS\Wan2GP
#
# You can override the parent Wan2GP path with:
#
#   set WAN2GP_ROOT=G:\APPS\Wan2GP
#

PROJECT_ROOT = Path(__file__).resolve().parent
WAN2GP_ROOT = Path(os.getenv("WAN2GP_ROOT", PROJECT_ROOT.parent)).resolve()

# Required so this subfolder can import the original Wan2GP modules,
# especially shared.api.
if str(WAN2GP_ROOT) not in sys.path:
    sys.path.insert(0, str(WAN2GP_ROOT))

# Required so local modules next to this file are importable even if the
# current working directory is not PROJECT_ROOT.
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from shared.api import init

from wan2gp_studio import create_studio_router
from wan2gp_studio_editor import create_studio_editor_router

# =========================================================
# CONFIG
# =========================================================

# Runtime files live inside the Git project folder, not in the original Wan2GP sources.
# Add runtime/ to .gitignore.
RUNTIME_DIR = Path(
    os.getenv("DGR_STUDIO_RUNTIME_DIR", PROJECT_ROOT / "runtime")
).resolve()

OUTPUT_DIR = Path(
    os.getenv("DGR_STUDIO_OUTPUT_DIR", RUNTIME_DIR / "api_outputs")
).resolve()

INPUT_DIR = Path(
    os.getenv("DGR_STUDIO_INPUT_DIR", RUNTIME_DIR / "api_inputs")
).resolve()

STUDIO_ASSETS_DIR = Path(
    os.getenv("DGR_STUDIO_ASSETS_DIR", RUNTIME_DIR / "studio_assets")
).resolve()

STUDIO_RENDERS_DIR = Path(
    os.getenv("DGR_STUDIO_RENDERS_DIR", RUNTIME_DIR / "studio_renders")
).resolve()

# Un seul template universel.
# Place-le idéalement ici :
#
#   dgr_agentic_studio\templates\ltx2_template_universal.json
#
# Tu peux aussi forcer son chemin avec :
#
#   set WAN2GP_TEMPLATE=G:\APPS\Wan2GP\dgr_agentic_studio\templates\ltx2_template_universal.json
#
TEMPLATE_FILE = Path(
    os.getenv(
        "WAN2GP_TEMPLATE",
        PROJECT_ROOT / "templates" / "ltx2_template_universal.json",
    )
).resolve()

API_TOKEN = os.getenv("WAN2GP_API_TOKEN", "HGH7EPBCE51vureBCBUEBCE75678edfv9HUGBC7E")

FIXED_MODEL_TYPE = os.getenv("WAN2GP_MODEL_TYPE", "ltx2_22B_distilled_1_1")
FIXED_BASE_MODEL_TYPE = os.getenv("WAN2GP_BASE_MODEL_TYPE", "ltx2_22B")
DISPLAY_MODEL_NAME = os.getenv("WAN2GP_DISPLAY_MODEL_NAME", "LTX-2 2.3 Distilled 1.1 22B")
...
MONITOR_TOKEN = os.getenv("WAN2GP_MONITOR_TOKEN", API_TOKEN)
MONITOR_TITLE = os.getenv("WAN2GP_MONITOR_TITLE", "Wan2GP Queue Monitor V2")
MONITOR_AUTO_REFRESH_SECONDS = int(os.getenv("WAN2GP_MONITOR_AUTO_REFRESH_SECONDS", "5"))
...
STUDIO_RENDERS_DIR.mkdir(parents=True, exist_ok=True)
...
def load_universal_template_settings(template_file: Path) -> dict:
    if not template_file.exists():
        raise RuntimeError(
            f"Template universel introuvable : {template_file}\n"
            f"Place-le dans : {PROJECT_ROOT / 'templates' / 'ltx2_template_universal.json'}\n"
            "Ou définis la variable WAN2GP_TEMPLATE."
        )
...
def root_redirect():
    return RedirectResponse(url=f"/studio?token={quote(str(API_TOKEN))}")
...
app.include_router(
    create_studio_router(
        ...
        get_requester_info=get_requester_info,
        save_upload_path=save_upload_path,
        write_upload_to_disk=write_upload_to_disk,
        build_v2_settings=build_v2_settings,
        create_job_record=create_job_record,
        start_generation_thread=start_generation_thread,
        get_job_raw=get_job_raw,
        get_jobs_snapshot=get_jobs_snapshot,
        get_health_snapshot=get_health_snapshot,
        get_model_snapshot=get_model_snapshot,
    )
)

app.include_router(
    create_studio_editor_router(
        api_token=API_TOKEN,
        get_jobs_snapshot=get_jobs_snapshot,
        get_job_raw=get_job_raw,
        studio_assets_dir=STUDIO_ASSETS_DIR,
        studio_renders_dir=STUDIO_RENDERS_DIR,
    )
)
...
if __name__ == "__main__":
    uvicorn.run(
        app,
        host=os.getenv("DGR_STUDIO_HOST", "0.0.0.0"),
        port=int(os.getenv("DGR_STUDIO_PORT", "7861")),
        reload=False,
    )
